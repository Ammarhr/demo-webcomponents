...
<script>
  ...
  export let meterurl, dailyurl, monthlyurl;
  let items;
  let tableData;
  ...
  let offPeakOprand;
  let words;
  const [data, loading, error, get] = globalFetch(); // meterTable fetch
  ...
  onMount(() => {
    if (!$data.results && meterurl && dailyurl && monthlyurl) {
      if ($isSummaryAccountFlag.toLowerCase() !== "true") {
        get("token", meterurl).then(() => {
          if ($data && $data.EPFlag) {
            EP_Flag = $data.EPFlag;
          } else {
            EP_Flag = "";
  ...
  const handleSelectedMeter = (meterObject, i) => {
    ...
    if (selectedMeter) {
      ...
      if (AMI_Flag == "X") {
        dailyUsageGet("token", dailyurl);
      }

      // monthly chart api call
      monthlyUsageGet("token", monthlyurl);
    }
    // MiJurney event call
