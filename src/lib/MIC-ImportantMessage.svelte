...
<script>
  ...
  import { globalFetch } from "../js/store";
  ...
  const [data, loading, error, get] = globalFetch();
  onMount(() => {
    if (messageurl && !$data.messages) {
      get("$apiToken", messageurl);
    }
  });
...
{:else if $error}
  Error: {$error}
{:else if $data && $data.messages && state && state.messages && state.messages[0]}
