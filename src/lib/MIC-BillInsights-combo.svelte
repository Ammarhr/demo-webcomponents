...
<script>
  ...
  import { renderBarChart, renderRadialBar } from "../js/chart-options";
  ...
  const [data, loading, error, get] = globalFetch(); // store fetch
...
{#if $loading}
  <mic-loading />
{:else if   insightservices && tabsToggleArr.length == insightservices.length}
